...
void main() {
   ...
   vec2 st = gl_FragCoord.xy/u_resolution;
    ...
    float t = (mod(u_time*0.5, 2.)) ;
    float m = clamp(easeOutBack(t, 0. , 1., 1.5 ,.5), .0, 1.1);
    st = rotate(st, clamp(map(t, 0.0, 2.0, 0., PI*2.), 0., PI));
    outer = starSDF(st.yx, 6, .09);
    col += stroke(outer, .55 * m , .05);
