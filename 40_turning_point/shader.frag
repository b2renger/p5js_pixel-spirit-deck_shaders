...
void main() {
   ...
   vec2 st = gl_FragCoord.xy/u_resolution;
    ...
    float t = (clamp(mod(u_time*.5,3.),1., 2.)-1.);
    float speed = easeOutCirc(t, 0., 1., 1.);
    ...
    col1 += stroke(top,.4,.15);
    col1 = bridge(col1, down, .4,.15);

    col2 += stroke(down,.4,.15);
    col2 = bridge(col2, top,.4,.15);
    vec3 col;
    col = mix(col1, col2, (cos(u_time)*0.5 +0.5 ));
