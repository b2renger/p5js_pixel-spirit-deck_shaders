...
void main() {
   ...
   vec2 st = gl_FragCoord.xy/u_resolution;
    ...
    vec3 col = vec3(0.);
    st = st.yx;
    st = rotate(st, -u_time *0.15);
    col += stroke(hexSDF(st), .6, .1);

    
    float t1 = mod(u_time, 10.);
    float t = map(sin(u_time), -1., 1., 0.5, 1.);
    

    if(t1 > 4. && t1 < 6.){
       float tc = map(t1, 4., 6., 0., 2.);
       st = rotate(st, easeInOutExpo(tc, 0., PI*2., 2.));
    }

    col += fill(hexSDF(st - vec2(-.09, -.15)*t -vec2(-.015, -.025)   ), .15);

    if(t1 > 2. && t1 < 4.){
       float tb = map(t1, 2., 4., 0., 2.);
       st = rotate(st, easeInOutExpo(tb, 0., PI*2., 2.));
    }
    col += fill(hexSDF(st - vec2(-.09, .15)*t -vec2(-.015, .025)) , .15);

    if(t1 > 0. && t1 < 2.){
       float ta = map(t1, 0., 2., 0., 2.);
       st = rotate(st, easeInOutExpo(ta, 0., PI*2., 2.));
    }
    col += fill(hexSDF(st - vec2(.165, .0)*t -vec2(.0275, 0.)), .15);
