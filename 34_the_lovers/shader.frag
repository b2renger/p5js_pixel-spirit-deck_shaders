...
void main() {
   ...
   vec2 st = gl_FragCoord.xy/u_resolution;
    ...
    float speed =  abs(cos(sin(u_time*4.) + u_time*4.));
    float size =map(speed, 0., 1., 0.2, 0.5);
    col += fill(heartSDF(st), size);
   //st = rotate(st, u_time*0.1);
   float size2 = map(speed, 0., 1., 0.01, 0.035);
    col -= stroke(polySDF(st, 3), .15 , size2);
