...
void main() {
   ...
   vec2 st = gl_FragCoord.xy/u_resolution;
    ...
    vec3 col;
    vec2 st1 = rotate(st, - PI/4.);
    float v1 = vesicaSDF(st1,-.525 );

    vec2 st2 = rotate(st1,  PI/2.);
    st2 = st2.yx+vec2(.04,.04);

    float t = pow(sin(u_time*2.)*0.5 + .5,.035);
    float offset1 = map(t, 1. , 0. , -0.51, -0.61);

    float v2 = vesicaSDF(st2, offset1);

    float offset2 = map(t, 1. ,0. , 0.025, .35);
    col += stroke(v2,1., offset2 );

    float offset3 = map(noise(vec2(u_time )), 0., 1., -0.05, 0.05) ;
    vec2 st3 = vec2(st.x + offset3 , st.y);

    col += fill(v2,1.)*
             stroke(circleSDF(st3 ),.3,.05);
    col += fill(raySDF(st,50),.2)*
             fill(v1,1.25)*
             step(1.,v2);
