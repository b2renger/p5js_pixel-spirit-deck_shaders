...
float smoothedge(float v) {
    return smoothstep(0., 1. / u_resolution.x, v);
}
...
void main() {
   ...
   vec2 st = gl_FragCoord.xy/u_resolution;
    ...
    st = rotate(vec2(st.x,  st.y),  -PI/4.); 
   
    float o = .05 /* cos(u_time*2.) + 0.1*/ ;
    float t2 = sin(u_time ) *.5 + .5;
    float t = fract(mod(u_time,2.));

   

    float shape1 = (circleSDF(st -o , .25 ));
    float shape2 = (rectSDF2(st + vec2(-.5 -o, -.5 -o), vec2(.25, .25))); // secondary rect function
    float shape3 = (circleSDF(st +o , .25 ));
    float shape4 = (rectSDF2(st + vec2(-.5 +o, -.5 +o), vec2(.25, .25))); // secondary rect function

    float geo1 = fill(smoothedge(mix(shape3, shape4   , t2)), .2);
    float geo2 = fill(smoothedge(mix(shape1, shape2   , t2)) , .2);

    float final = flip(geo1, geo2);

    col += final;
   


     //col = vec3(smoothedge(col));
