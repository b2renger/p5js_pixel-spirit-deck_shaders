...
void main() {
   ...
   vec2 st = gl_FragCoord.xy/u_resolution;
    ...
    float t = sin(u_time *.5)+1.;
    float t0 = easeInOutExpo(t, .0, 1., .5) * PI;
    ...
    float t1 = clamp(pow(cos(u_time*0.5) + 1.5, 3.), 0.15, 1.) ;
    float t2 = clamp((sin(u_time)+1. ), .05,.72);
  
    for (float i = .0; i < 8. ; i++){
        vec2 xy = rotate(st, .39 + a*i);
        xy = scale(xy, vec2(t1, t2));
        xy.y -= .125;
