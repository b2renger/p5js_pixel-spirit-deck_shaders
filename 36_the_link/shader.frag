...
void main() {
   ...
   vec2 st = gl_FragCoord.xy/u_resolution;
    ...
    float t5 = clamp(t, 0., 1.);
    float rot = bounceEaseOut(t5, 0., .5, 1.) * PI*2.;
    //st = rotate(st, rot);
    ...
    float t4 = clamp(t, 3.5, 9.);
    float v = map(t4, 3.5, 9., 0., 0.1);
    vec2 o = vec2(0.1, 0.);

   
  
    float t2 = clamp(t, 1., 7.);
    float size = map(t2, 1., 7., 0., 1.2);
    ...
    float weight = .08;
    col += stroke(l, .3, weight);
    col = bridge(col, r, .3, weight);

    float t1 = clamp(t, 3., 7.) ;
    float pos = map(t1, 3., 7., 0., 0.5);
    float t3 = clamp(t, 0., 2.) /2.;
    col += fill(rhombSDF(abs(st.yx - vec2(.0,pos ))),0.1)*t3 ;
    col *=  step(.1, st.y);
