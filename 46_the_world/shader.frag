...
float vesicaSDF(vec2 st,float w){
    vec2 offset=vec2(.5*w,.0);
    return max(circleSDF(st-offset),circleSDF(st+offset));
}

float vesicaSDF2(vec2 st,float u){
    vec2 offset=vec2(.5*u);
...
float triSDF2(vec2 st,float h){
    st=(st*2.-1.)*2.;
    return max(abs(st.x)*.866025+st.y*h,-st.y*h);
}
    ...
    float random(vec2 st){
        return fract(sin(dot(st.xy,
            ...
            void main(){
                ...
                float t=clamp(sin(u_time)*2.,-1.,1.);
                float t1=sin(u_time)*.5+.5;
                float t2=sin(u_time)*.5+.5;
                
                vec2 p=vec2(clamp((1.-(pow(sin(u_time)*.5+.5,.25)))*5.,1.55,1.));
                
                st=rotate(st,t2);
                
                st=scale(st,p);
                
                col+=fill(flowerSDF(st,5),.25);
                col-=step(.95,starSDF(rotate(st,.628),5,.1*t));
                col=clamp(col,0.,1.);
                
                float c=circleSDF(st);
                col-=stroke(c,.1*t1,.05*t1);
                
                st=scale(st,1./p);
                c=circleSDF(st);
                col+=stroke(c,.8,.06+.01);
