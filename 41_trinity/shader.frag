...
float polySDF2(vec2 st, float V){
    st = st*2. -1.;
    float a = atan(st.x, st.y) + PI ;
    float r = length(st);
    float v = 2.*PI / float(V);
...
void main() {
   ...
   vec2 st = gl_FragCoord.xy/u_resolution;
    ...
    float time1 = clamp(mod(u_time*1.25, 10.), 0., 5.);
    float time2 = clamp(mod(u_time*1.25, 10.), 0., 8.);
    float time3 = clamp(mod(u_time*1.25, 10.), 5., 9.);
    ...
    n = map(time1, 0., 5., 9., 3.);
    size = map(pow(easeOutBack(time2, 0., 8., 8.,1.),.85) , 0., pow(8., 0.85), .9, .3);
    width = map(time3, 5., 9., 0.06, 0.1);
    int nsides = int(n);
  

    float t2 = polySDF(st+vec2(.1, .0),nsides);
    float t3 = polySDF(st-vec2(.1, .0),nsides);
    float t1 = polySDF(st+vec2(.0, .175),nsides);


    
    col1 += stroke(t1, size, width) +
            stroke(t2, size, width) +
            stroke(t3, size, width);
    ...
    col1 = bridge(col1, bridges1,size,width);
